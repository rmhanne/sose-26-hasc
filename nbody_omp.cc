...
// vectorized acceleration computation exploiting symmetry
void acceleration (int n, double3* __restrict__ x, double* __restrict__ m, double3* __restrict__ aglobal)
{
...
#pragma omp parallel shared(aglobal), firstprivate(n,x,m) private(Ai,D0,D1,D2,D3,A0,A1,A2,A3,S,U,T0,T1,T2)
  {
    // make private acceleration vector to accumulate to
    std::vector<std::array<double,M>> a(n,{0.0,0.0,0.0,0.0});
